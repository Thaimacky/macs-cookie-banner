...
- 2026-08-06 — **v1.0.7 (Google Tracking Shield — kritischer Hotfix, ADR-40).** Schliesst die bewiesene Lücke, dass GA4/GTM/Google-Ads-Scripts unabhängig von der Einbindungsquelle (Avada, Google Site Kit, Rank Math, Theme-Header-Feld, Custom Code) vor Consent laden konnten, weil die bisherigen Gating-Module (`script_loader_tag`) nur `wp_enqueue_script()`-registrierte Assets sehen. Neues opt-in Modul `includes/google-tracking-shield.php`: Output-Buffer auf `template_redirect` (nur echte HTML-Frontend-Responses; kein Eingriff in Admin/REST/AJAX/Cron/Feeds/Robots/Trackbacks), zwei Ebenen — `WP_HTML_Tag_Processor` (WP-Core seit 6.2) für externe `<script src>`-Tags (primär), eng auf Tag-Grenzen begrenzte Erkennung für zugehörigen Inline-Bootstrap-Code (sekundär, nur bei vollständiger, eindeutiger Klassifikation, keine breite Wort-Blockade). Reaktivierung über die unveränderte `banner.js`-Mechanik. `match_vendor()` zusätzlich um Legacy Universal Analytics erweitert. **ADR-36-AUSNAHME (bewusst, einmalig, dokumentiert):** anders als jede sonstige Safe-by-Default-Option wird der Schutz bei bestehenden Installationen beim Update einmalig automatisch aktiviert (bewiesene aktive Compliance-Lücke rechtfertigt die Ausnahme von „Bestand nie ungefragt ändern"); danach nie wieder überschrieben, administrativer Not-Aus vorhanden (Checkbox + `MCB_DISABLE_GOOGLE_SHIELD`). Bestehende Compat-Module unverändert. Kein Teil der vollständigen „Universal Tracking Protection"-Architektur (separate Studie, nicht umgesetzt).
...
- **Priorität 1:** Google Analytics 4, Google Tag Manager, Meta Pixel, YouTube Embeds, Google Maps, Google Ads / Conversion / Remarketing.
  - **GA4 / GTM / Google Ads — ab v1.0.7 quellenunabhängig gegated** (`google-tracking-shield.php`, ADR-40), unabhängig davon, welches Plugin/Theme/Builder den Code einbindet. Meta Pixel läuft weiterhin über den Consent-Code-Manager (manuell); YouTube/Maps über die bestehenden MCB-Komponenten/Shortcodes/Avada-Module.
...
- **Separate Architekturstudie „Universal Tracking Protection"** (2026-08-06, nicht umgesetzt) bewertet alle WordPress-/Browser-Ebenen für eine vollständig quellenunabhängige Lösung über GA4/GTM/Ads hinaus; v1.0.7 setzt daraus bewusst nur den eng begrenzten, dringenden Ausschnitt um.
