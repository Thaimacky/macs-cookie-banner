...
| `includes/admin-page.php` | Admin-Menue, Einstellungsseite, Speichern via `admin-post.php` mit Nonce-Pruefung, Render-Helpers fuer Text- und Color-Felder. Ab 0.5.1: Button „Avada-Farben übernehmen" + Handler `import_avada_colors()` (admin-post `mcb_import_avada_colors`). |
| `includes/avada-colors.php` | Avada-Farbimport (ab 0.5.1), rein lesend, admin-only, **keine** Hooks. Klasse `Macs_Cookie_Banner_Avada_Colors`: ermittelt **eine** Markenfarbe via Prioritätskette `primary_color → accent_color → link_color → button_gradient_top_color` (erster gültiger Hit; `var(--awb-colorN)` über Palette aufgelöst) und mappt sie auf `primary_button_color` + `border_color`; `primary_text_color` = WCAG-Auto-Kontrast. Import nur auf bewussten Klick (ADR-27); kein Auto-Sync, kein Avada-Schreibzugriff, kein Versions-/Legacy-Pfad. Ab 0.5.9: `reset_caches()` leert nach dem Import den Avada/Fusion-Cache über Avadas eigene API (`fusion_reset_all_caches()` → `Fusion_Cache::reset_all_caches()`), damit das gecachte Inline-CSS die neue Farbe sofort zeigt (ADR-29). |
