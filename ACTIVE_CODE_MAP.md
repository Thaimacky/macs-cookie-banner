...
>
> **Google Tracking Shield (ab v1.0.7, ADR-40):** quellenunabhängiges Output-Buffer-Gating für GA4/GTM/Google-Ads-`<script>`-Tags (siehe `includes/google-tracking-shield.php` unten) — schliesst die bewiesene Lücke, dass bisherige Gating-Module nur `wp_enqueue_script()`-registrierte Assets sehen, nicht roh geechote Tags (Google Site Kit u. ä.). **ADR-36-Ausnahme:** bei bestehenden Installationen beim Update auf 1.0.7 einmalig automatisch aktiviert (Compliance-kritisch), danach nie wieder überschrieben.
...
| `includes/google-tracking-shield.php` | Google Tracking Shield (ab 1.0.7, ADR-40): quellenunabhängiges Output-Buffer-Gating für GA4/GTM/Google-Ads-`<script>`-Tags, unabhängig davon, ob Avada, Google Site Kit, Rank Math, ein Theme-Header-Feld oder Custom Code sie einbindet. Registriert bei aktivierter Option `google_tracking_shield` einen Buffer auf `template_redirect` (Guards: `is_admin`/AJAX/Cron/REST/XML-RPC/JSON/Feed/Robots/Trackback → kein Buffer-Start; zusätzlicher Content-Sniff auf `<!doctype`/`<html` im Callback selbst). Layer A `gate_external_scripts()`: `WP_HTML_Tag_Processor` (WP-Core seit 6.2), reine Attribut-Ebene, `src` gegen `Consent_Codes::match_vendor()` geprüft, bei `ga4`/`gtm`/`google_ads` → `type="text/plain"` + `data-cookie-category`/`data-cookie-type`. Layer B `gate_inline_bootstraps()`: Tag-Grenzen-verankerte `preg_replace_callback` für reine Inline-Scripts (kein `src`), nur bei vollständiger `match_vendor()`-Klassifikation des **gesamten** Blocks, Rewrite über das bestehende `Consent_Codes::transform_snippet()`. Beide Layer überspringen `<script>`-Tags mit nicht-JS `type` (z. B. `application/ld+json`) hart. Reaktivierung ausschliesslich über die unveränderte `banner.js::activateBlockedScripts()`. Nur Frontend, opt-in via `google_tracking_shield`; **ADR-36-Ausnahme:** bei bestehenden Installationen beim Update auf 1.0.7 einmalig automatisch aktiviert (`Macs_Cookie_Banner::maybe_migrate_google_shield()`, Versions-Stamp `mcb_google_shield_migrated`, danach nie wieder überschrieben). Not-Aus: Settings-Checkbox + `wp-config`-Konstante `MCB_DISABLE_GOOGLE_SHIELD`. |
...
- `MCB_DISABLE_GOOGLE_SHIELD` (ab 1.0.7, Default `false`, kann via `wp-config.php` ueberschrieben werden) — unabhängiger Not-Aus für den Google Tracking Shield (ADR-40), zusätzlich zur Settings-Checkbox
...
- **Safe-by-Default (ab 1.0.3, ADR-36; erweitert 1.0.4/1.0.7):** `get_recommended_defaults()` = sichere Werte (Quelle für Fresh-Install-Seeding **und** Restore; `avada_code_maps_block`/`avada_youtube_block`/`meta_social_block`/`google_tracking_shield`/`show_legal_links`=true, `youtube_remote_thumbnails`=false, `consent_lifetime_days`=180; `avada_maps_block`/`yotu_consent_gating` bewusst **nicht** im Set). `get_baseline_fallback()` = konservative Struktur-Defaults; **`sanitize_options()` nutzt für fehlende Keys den Baseline-Fallback** → Updates aktivieren neue Schutzoptionen nie still. `get_restore_recommended_keys()` = die vom Restore betroffenen Keys. `on_activate()` seedet `get_recommended_defaults()` via `add_option` **nur** bei fehlendem Options-Eintrag (Fresh-Install); Bestand unberührt.
- **`maybe_migrate_google_shield()` (ab 1.0.7, ADR-40, private, aufgerufen am Anfang von `init()`):** einzige, ausdrücklich dokumentierte **Ausnahme** von der Safe-by-Default-Regel „Bestand wird nie ungefragt verändert" — aktiviert `google_tracking_shield` bei **bestehenden** Installationen **einmalig** automatisch (Versions-Stamp-Option `mcb_google_shield_migrated` gate't die Methode dauerhaft; danach nie wieder Schreibzugriff, auch nicht nach künftigen Updates). Läuft in jedem Request-Kontext (Frontend **und** Admin), nicht nur `admin_init`, um die Lücke ab dem nächsten Seitenaufruf zu schliessen statt erst beim nächsten wp-admin-Besuch.
...
- `get_privacy_url( $options )` — manueller Override hat Vorrang (durch `localize_url()` geführt), sonst auf mehrsprachigen Sites die Core-Privacy-Seite direkt über `wp_page_for_privacy_policy` → `translate_post_id()` → `get_permalink()`, einsprachig der unveränderte `get_privacy_policy_url()`-Pfad. **Ab 1.0.6 (ADR-39) sprachabhängig.**
- `get_imprint_url( $options )` — manueller Override hat Vorrang (durch `localize_url()` geführt), sonst Transient `lscc_detected_imprint_url` (ebenfalls durch `localize_url()`). Frontend liest ausschliesslich den Cache. **Ab 1.0.6 (ADR-39) sprachabhängig.**
- `localize_url( $url )` — privat, **ab 1.0.6 (ADR-39)**: löst eine interne Seiten-URL beim Rendern in die aktive Sprache auf (`url_to_postid()` → `translate_post_id()` → `get_permalink()`); No-op ohne Multilingual-Plugin; externe/nicht auflösbare URLs und fehlende Übersetzungen → Original-URL. **Keine** URL-String-Manipulation.
- `translate_post_id( $post_id )` — privat, **ab 1.0.6 (ADR-39)**: übersetzt eine Post-ID in die aktive Sprache. WPML `apply_filters( 'wpml_object_id', $id, get_post_type($id), true )` (Original-Fallback), Polylang `pll_get_post( $id )` (falsy → Original); ohne Plugin Original-ID.
- `is_multilingual()` — privat, **ab 1.0.6 (ADR-39)**: `has_filter('wpml_object_id') || function_exists('pll_get_post')`. Guard, der die gesamte Sprach-Auflösung auf einsprachigen Sites kurzschließt.
...
- `detect_vendor( $code )` / `match_vendor( $text )` — Mustererkennung des Drittanbieters (für Scanner/Badge). **Ab 1.0.1:** `google_ads` (AW-…/`google_conversion_id`/`googleadservices.com`/`googleads.g.doubleclick.net`) wird **vor** GA4 geprüft (Ads nutzt ebenfalls `gtag()`; sonst GA4-Fehlklassifikation); `mailchimp` (`chimpstatic.com`/`list-manage.com`/`mailchimp`). **Ab 1.0.4:** `facebook_embed` (`facebook.com/plugins/`, `connect.facebook.net`+`sdk.js`, `fbAsyncInit`, `FB.init`, `fb-page/-post/-video`) und `instagram_embed` (`instagram.com/embed`, `platform.instagram.com`, `instagram-media`, `instgrm`) werden **vor** `meta_pixel` geprüft → strikte Trennung Social↔Pixel; `meta_pixel` (`fbevents.js`/`fbq`) unverändert. Default-Kategorie beider Social-Vendoren = `external_media`. **Ab 1.0.5:** `brevo` (`brevo.com`/`sendinblue`/`sibautomation.com`/`sibforms.com`/`window.sib`/`window.brevo`/`sib-tracker`/`sib-form`/`sib-container`/`sibwp_form`, Default-Kategorie marketing); `recaptcha`-Erkennung erweitert (`gstatic.com/recaptcha`, `recaptcha/api.js`, `recaptcha/enterprise.js`). reCAPTCHA wird **nur erkannt/gemeldet, nicht automatisch geblockt** (Formular-Bruch-Risiko, ADR-38). **Ab 1.0.7 (ADR-40):** `ga4`-Zweig erweitert um Legacy Universal Analytics (`google-analytics.com/analytics.js`, `google-analytics.com/ga.js`) — vorher unerkannt, gleiche Kategorie `statistics` wie GA4.
