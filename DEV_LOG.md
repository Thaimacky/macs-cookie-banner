...
- **Scope (bewusst eng, KEIN „Universal Tracking Protection"-Rollout):** ausschliesslich eindeutig erkennbares GA4/GTM/Google Ads. Kein neues Compat-Modul entfernt/umgebaut (`avada-compat`, `avada-maps-compat`, `avada-code-compat`, `meta-social-compat`, `yotu-compat` unverändert).
- **Neues Modul `includes/google-tracking-shield.php`** (Klasse `Macs_Cookie_Banner_Google_Tracking_Shield`), opt-in via `google_tracking_shield`:
  - Output-Buffer auf `template_redirect`, nur wenn Option aktiv und `MCB_DISABLE_GOOGLE_SHIELD` nicht gesetzt. Kein Buffer-Start bei `is_admin()`/AJAX/Cron/`REST_REQUEST`/`XMLRPC_REQUEST`/`DOING_CRON`/`DOING_AJAX`/`wp_is_json_request()`/Feed/Robots/Trackback. Zusätzlicher, unabhängiger Content-Sniff im Callback (`<!doctype`/`<html`) als zweite Schutzschicht (deckt z. B. XML-Sitemaps ab).
  - **Layer A `gate_external_scripts()`** (primär): `WP_HTML_Tag_Processor` (WP-Core seit 6.2), reine Attribut-Ebene — jedes `<script>`-Opening-Tag, `src` gegen `Consent_Codes::match_vendor()` geprüft, bei `ga4`/`gtm`/`google_ads` → `type="text/plain"` + `data-cookie-category`/`data-cookie-type`. Deckt den gemeldeten Fall direkt und vollständig ab.
  - **Layer B `gate_inline_bootstraps()`** (sekundär, eng begrenzt): Tag-Grenzen-verankerte `preg_replace_callback` für reine Inline-`<script>`-Blöcke (kein `src`); gated nur, wenn der **gesamte** getrimmte Inhalt über `match_vendor()` eindeutig als `ga4`/`gtm`/`google_ads` klassifiziert — keine Blockade allein wegen „google"/„dataLayer"/„analytics"/„collect". Rewrite über das bestehende `Consent_Codes::transform_snippet()` (keine zweite Transform-Logik). Deckt zusätzlich den GTM-Container-Snippet-Fall ab (die selbstnachladende IIFE liegt als Inline-Block vor und wird komplett verhindert, bevor sie ausgeführt wird — nicht nur das dynamisch nachgeladene `gtm.js`).
  - **False-Positive-Schutz (beide Layer):** `<script>`-Tags mit explizitem, nicht-JS `type` (z. B. `application/ld+json` — Rank-Math-Strukturdaten) werden nie angefasst.
  - Reaktivierung ausschliesslich über die bestehende, unveränderte `banner.js::activateBlockedScripts()` — kein neuer Frontend-Code.
- **`includes/consent-codes.php`:** `match_vendor()`-`ga4`-Zweig um Legacy Universal Analytics erweitert (`google-analytics.com/analytics.js`, `google-analytics.com/ga.js` — war zuvor unerkannt, nötig für den im Auftrag geforderten Scope). Kategorie weiterhin `statistics`.
- **`macs-cookie-banner.php`:** neue Option `google_tracking_shield` (bool) in `get_default_options()`/`get_bool_option_keys()`; in `get_recommended_defaults()`/`get_restore_recommended_keys()` aufgenommen (Fresh-Install + Restore-Button = EIN, wie jede andere Safe-by-Default-Option). Neue Konstante `MCB_DISABLE_GOOGLE_SHIELD` (Default false, wp-config-überschreibbar, Not-Aus unabhängig von der Admin-UI). `require_once` + `init()` des neuen Moduls (nach `consent-codes.php`, da abhängig von `match_vendor()`/`transform_snippet()`/`vendor_default_category()`).
  - **ADR-36-AUSNAHME (bewusst, dokumentiert):** neue private Methode `maybe_migrate_google_shield()`, aufgerufen am Anfang von `init()` (jeder Request-Kontext, Frontend **und** Admin). Aktiviert `google_tracking_shield` bei **bestehenden** Installationen **einmalig automatisch** — anders als jede andere Safe-by-Default-Option in diesem Plugin, wo Bestand nie ungefragt verändert wird. Begründung: bewiesene, aktive Compliance-Lücke; das sonstige „nie ungefragt" würde sie auf allen Bestandsinstallationen unbegrenzt offen lassen. Gate über dedizierten Versions-Stamp `mcb_google_shield_migrated` (String-Option, einmal gesetzt → Methode läuft nie wieder). Ein Betreiber, der danach manuell deaktiviert und speichert, wird nie wieder überschrieben. Fresh-Installs unverändert über den regulären `on_activate()`-Pfad.
- **`includes/admin-page.php`:** neue Sektion „Google-Tracking-Schutz (GA4 / GTM / Google Ads)" mit Checkbox `google_tracking_shield` (Reuse `render_checkbox_field`) + Beschreibung inkl. explizitem ADR-40-Hinweis auf die einmalige Migration. „Sichere Standardwerte"-Infobox und Restore-Erfolgsmeldung um den neuen Schalter ergänzt.
- `macs-cookie-banner.php`: Version 1.0.6 → **1.0.7** (Header + `MCB_VERSION`). `MCB_CONSENT_VERSION` unverändert (Consent-Datenstruktur ändert sich nicht).
- **Validierung (kein lokaler PHP-Runtime — dokumentierte Projektkonstante):**
  - Klammer-/Klammerpaar-Balance aller vier geänderten/neuen Dateien per Skript geprüft (`{}`/`[]` exakt balanciert; die rohe `()`-Zählung zeigt in `admin-page.php`/`consent-codes.php` dieselbe Differenz wie bereits VOR dieser Änderung — bestätigt Prosa-Klammern in Doku-/UI-Text, kein neuer Fehler).
  - Logik-Trace: 1:1-Python-Port der neuen Klassifikations-/Rewrite-Regeln (`match_vendor`, `vendor_default_category`, `transform_snippet`, Layer A/B) gegen 7 Fixtures — reales Bug-Beispiel (GA4/gtag.js), GTM-Container-Snippet, Google-Ads-Snippet, JSON-LD-Negativtest, „dataLayer"-Keyword-Negativtest, Legacy-UA, Meta-Pixel/Brevo/reCAPTCHA-Negativtest. Alle 7 bestanden. **Wichtiger Vorbehalt:** Layer A nutzt in PHP real `WP_HTML_Tag_Processor` (WP-Core-Parser); der Python-Trace bildet dessen erwartetes Attribut-Rewrite-Verhalten nach, ersetzt aber keinen echten WP-Lauf.
  - Echter Browser-/Netzwerk-/Speicher-/Page-Cache-Test **nicht in dieser Umgebung möglich** (kein lokales PHP/WordPress/Browser verfügbar) — Runtime-Proof erfolgt auf echter WP-Installation gemäss `RELEASE_CHECKLIST.md`, wie bei jeder vorherigen Version dieses Projekts.
- **Nicht angefasst:** `banner.js`, Consent-Modell/-Kategorien, `MCB_CONSENT_VERSION`, Avada-Module, Meta-Social-Modul, YOTU-Modul, Consent-Code-Manager-UI/-Speicherlogik, Privacy Check, Scanner-Architektur.
- **Hinweis zur Versionierung:** ursprünglich als v1.0.6 begonnen; beim Push wurde festgestellt, dass v1.0.6 auf dem Remote bereits durch ein anderes, unabhängiges Feature (WPML/Polylang-Rechtslinks, siehe unten) belegt ist. Sauber auf v1.0.7 rebased, ADR-Nummer von 39 auf 40 verschoben; inhaltlich sonst unverändert.
