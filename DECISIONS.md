...
**Entscheidung (eng begrenzter Hotfix, keine „Universal Tracking Protection"):**
- **Neues Modul `includes/google-tracking-shield.php`**, opt-in via Option `google_tracking_shield`. Registriert bei aktivierter Option einen Output-Buffer auf `template_redirect` (Layer, nicht Reichweite, siehe unten).
- **Guard vor dem Buffer-Start:** `is_admin()`, `wp_doing_ajax()`, `wp_doing_cron()`, `REST_REQUEST`, `XMLRPC_REQUEST`, `DOING_CRON`, `DOING_AJAX`, `wp_is_json_request()`, `is_feed()`, `is_robots()`, `is_trackback()` — alle führen zu keinem Buffer-Start. Zusätzlicher, unabhängiger zweiter Schutz im Buffer-Callback selbst: verarbeitet wird nur, wenn der Puffer plausibel mit `<!doctype`/`<html` beginnt (deckt z. B. XML-Sitemaps ab, die keinen der expliziten Guards treffen).
- **Layer A (primär, `gate_external_scripts()`):** `WP_HTML_Tag_Processor` (WP-Core seit 6.2) rein auf Attribut-Ebene — jedes `<script>`-Opening-Tag wird durchlaufen, `src` gegen `Consent_Codes::match_vendor()` geprüft; bei Treffer `ga4`/`gtm`/`google_ads` wird `type` auf `text/plain` + `data-cookie-category`/`data-cookie-type` umgeschrieben (identisches Format wie überall sonst im Plugin). Kein Rohtext-Parsing nötig — deckt den gemeldeten Fall direkt und vollständig ab.
- **Layer B (sekundär, eng begrenzt, `gate_inline_bootstraps()`):** reine Inline-`<script>`-Blöcke (kein `src`) werden nur gegated, wenn ihr **gesamter** getrimmter Inhalt über **dieselbe** `match_vendor()`-Engine eindeutig als `ga4`/`gtm`/`google_ads` klassifiziert wird — **keine** lose Volltext-Ersetzung, **keine** Blockade allein wegen der Wörter „google"/„dataLayer"/„analytics"/„collect". Tag-Grenzen-verankerte `preg_replace_callback` (`</script>` kann per HTML-Spec nicht im Script-Body vorkommen), dieselbe Technikklasse wie bereits `Consent_Codes::transform_snippet()`/`Avada_Code_Compat`. Rewrite läuft über das bestehende `transform_snippet()` — keine zweite Transform-Logik. Deckt zusätzlich den GTM-Container-Snippet-Fall ab (die IIFE, die `gtm.js` erst zur Laufzeit per `document.createElement` nachlädt, liegt selbst als Inline-Block vor und wird durch die Gating dieses Blocks vollständig verhindert, bevor sie je ausgeführt wird).
- **Harte False-Positive-Schutzschicht (beide Layer):** `<script>`-Tags mit einem expliziten, nicht-JS `type` (z. B. `application/ld+json` — Rank-Math-Strukturdaten) werden **nie** angefasst, unabhängig vom Inhalt.
- **`match_vendor()`-Erweiterung (`includes/consent-codes.php`):** zwei neue Needles im `ga4`-Zweig (`google-analytics.com/analytics.js`, `google-analytics.com/ga.js`) für Legacy Universal Analytics — kleine, präzise Ergänzung der bestehenden, bereits produktiv genutzten Engine, keine neue Vendor-Kategorie.
- **Reaktivierung:** ausschliesslich über die bestehende, unveränderte `banner.js::activateBlockedScripts()`-Sequenz — keine zweite Consent-Engine, keine neue Frontend-Logik.

**ADR-36-AUSNAHME (bewusst, einmalig, dokumentiert):** Anders als jede andere Safe-by-Default-Schutzoption in diesem Plugin wird `google_tracking_shield` bei bereits **bestehenden** Installationen beim Update auf 1.0.7 **einmalig automatisch** aktiviert (`Macs_Cookie_Banner::maybe_migrate_google_shield()`, läuft in jedem Request-Kontext, gate-checked über den dedizierten Versions-Stamp `mcb_google_shield_migrated`). Begründung: die Schutzoption schliesst eine **bewiesene, aktive Compliance-Lücke**; das sonst geltende „Bestand wird nie ungefragt verändert" (ADR-36) würde diese Lücke auf allen bestehenden Produktivinstallationen unbegrenzt offen lassen, bis ein Betreiber zufällig manuell opt-in geht — für einen Compliance-kritischen Hotfix nicht vertretbar. Migration läuft **genau einmal**, für immer (Versions-Stamp-Gate); ein Betreiber, der danach manuell deaktiviert und speichert, wird **nie wieder** automatisch überschrieben. Fresh-Installs erhalten den Schutz weiterhin über den regulären, unveränderten ADR-36-Pfad (`get_recommended_defaults()` → `on_activate()`).
- **Administrativer Not-Aus:** Checkbox in den Plugin-Einstellungen (immer erreichbar, da wp-admin nie gebuffert wird) + zusätzliche, unabhängige `wp-config.php`-Konstante `MCB_DISABLE_GOOGLE_SHIELD` als Verteidigung in der Tiefe.

**Bewusst NICHT:** keine generische Vendor-Liste über GA4/GTM/Ads hinaus, kein browserseitiger Netzwerk-Guard, kein CSP, keine Umgestaltung/Entfernung der fünf bestehenden Compat-Module (`avada-compat`, `avada-maps-compat`, `avada-code-compat`, `meta-social-compat`, `yotu-compat` — bleiben unverändert bestehen), keine zweite Consent-Engine, kein `MCB_CONSENT_VERSION`-Bump (Struktur des gespeicherten Consents ändert sich nicht).

**Bekannte, dokumentierte Grenze:** Tracking, das ausschliesslich zur Laufzeit im Browser dynamisch nachgeladen wird **ohne** dass der auslösende Code selbst als literaler `<script>`-Block im Server-Response vorliegt (z. B. via einer bereits erlaubten, unabhängigen Drittscript-Kette), sowie serverseitig/edge-seitig injiziertes Tracking ausserhalb von WordPress, bleiben unerfasst — deckungsgleich mit der bereits dokumentierten Restlücke der separaten „Universal Tracking Protection"-Architekturstudie. Erfasst wird jeder Fall, in dem der GA4-/GTM-/Ads-Ladecode selbst literal in der ausgelieferten HTML-Antwort steht (der ganz überwiegende Teil realer Integrationen, einschliesslich Google Site Kit, GTM-Plugins, Avada-Header-Feldern und des GTM-Container-Snippets samt dessen eigener Laufzeit-Nachladelogik).

**Folgen / offene Punkte:** `MCB_CONSENT_VERSION` unverändert. Runtime-Proof (Netzwerk-Traffic, Speicher, Page-Cache-Kompatibilität, Regressions-Matrix) erfolgt auf einer echten WordPress-Installation gemäss `RELEASE_CHECKLIST.md` — hier keine lokale PHP-/Browser-Ausführung möglich (Projektkonstante, siehe DEV_LOG). Verbindliche Referenz: `MASTER_HANDBUCH.md`.
